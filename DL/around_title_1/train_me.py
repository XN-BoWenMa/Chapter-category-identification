...
class data_prefetcher():
    ...
    def preload(self):
        try:
            self.next_target, self.next_title, self.next_before, self.next_after = next(self.loader)
    ...
    def next(self):
        ...
        self.preload()
        return target, title, before, after
...
def get_args():
    ...
    parser.add_argument("--train_set", type=str, default=path+"around_title_1_train.csv")
    parser.add_argument("--test_set", type=str, default=path+"around_title_1_valid.csv")
...
def train(opt):
    ...
    for epoch in range(opt.num_epoches):
        ...
        label, title, before, after = prefetcher.next()
        iter = 0
        ...
        # for iter, (feature, label, title, before, after) in enumerate(training_generator,1):
            ...
            predictions = model(title, before, after)
        ...
        if epoch % opt.test_interval == 0:
            ...
            te_label_ls = []
            te_pred_ls = []
            for te_label, te_title, te_before, te_after in test_generator:
                ...
                with torch.no_grad():
                    ...
                    te_predictions = model(te_title, te_before, te_after)
            ...
            if te_loss + opt.es_min_delta < best_loss:
                ...
                torch.save(state, opt.saved_path + os.sep + "around_title_1.pkl")
            else:
