...
class MyDataset(Dataset):
    ...
    def __init__(self, data_path, dict_path, max_length_sentences=30, max_length_word=35):
        ...
        labels, around_before, around_after, titles = [], [], [], []
    ...
    def __getitem__(self, index): #获取每一条数据 #并进行编码
        ...
        after_encode = []
        if len(after)>0:
            after_encode = [self.dict.index(word) if word in self.dict else -2 for word in after]
            if len(after_encode) < 4:
                extended_words = [-1 for _ in range(4-len(after_encode))]
                after_encode.extend(extended_words)
            else:
                after_encode = after_encode[:4]
        else:
            after_encode = [-1 for _ in range(4)]
        after_encode = [i+2 for i in after_encode]
        #---------------------------------------------------------
        ...
        return label, np.array(title_encode).astype(np.int64), np.array(before_encode).astype(np.int64), np.array(after_encode).astype(np.int64)
