...
class MyDataset(Dataset):
    ...
    def __init__(self, data_path, dict_path, max_length_sentences=30, max_length_word=35):
        ...
        labels, around_before1, around_before2, around_after1, around_after2, titles = [], [], [], [], [], []
    ...
    def __getitem__(self, index): #获取每一条数据 #并进行编码
        ...
        return label, np.array(title_encode).astype(np.int64), np.array(before_encode1).astype(np.int64), np.array(before_encode2).astype(np.int64), np.array(after_encode1).astype(np.int64), np.array(after_encode2).astype(np.int64)
