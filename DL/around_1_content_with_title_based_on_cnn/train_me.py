...
class data_prefetcher():
    ...
    def preload(self):
        try:
            self.next_input, self.next_target, self.next_before1, self.next_after1, self.title, self.before_title, self.after_title = next(self.loader)
            ...
            self.title = None
            self.before_title = None
            self.after_title = None
            return
        with torch.cuda.stream(self.stream):
            ...
            self.before_title = self.before_title.cuda(non_blocking=True)
            self.after_title = self.after_title.cuda(non_blocking=True)
            # With Amp, it isn't necessary to manually convert data to half.
            # if args.fp16:
    ...
    def next(self):
        ...
        title_before = self.before_title
        title_after = self.after_title
        self.preload()
        return input, target, before1,after1, title_current, title_before, title_after 
...
def get_args():
    ...
    parser.add_argument("--train_set", type=str, default=path+"around_1_content_with_title_train.csv")
    parser.add_argument("--test_set", type=str, default=path+"around_1_content_with_title_valid.csv")
    ...
    parser.add_argument("--filter_sizes_fuse", type=list, default=[1,2,3])
...
def train(opt):
    ...
    for epoch in range(opt.num_epoches):
        ...
        feature, label, before1, after1, title_current, title_before, title_after = prefetcher.next()
        iter = 0
        ...
        # for iter, (feature, label) in enumerate(training_generator):
            if torch.cuda.is_available():
                ...
                title_before = title_before.cuda()
                title_after = title_after.cuda()
            optimizer.zero_grad()
            model._init_hidden_state() #初始化隐藏层
            predictions = model(feature, before1, after1, title_current, title_before, title_after)
        ...
        if epoch % opt.test_interval == 0:
            ...
            te_label_ls = []
            te_pred_ls = []
            for te_feature, te_label, te_before1, te_after1, te_title_current, te_title_before, te_title_after in test_generator:
                ...
                if torch.cuda.is_available():
                    ...
                    te_title_before = te_title_before.cuda()
                    te_title_after = te_title_after.cuda()
                with torch.no_grad():
                    model._init_hidden_state(num_sample)
                    te_predictions = model(te_feature, te_before1, te_after1, te_title_current, te_title_before, te_title_after)
            ...
            if te_loss + opt.es_min_delta < best_loss:
                ...
                torch.save(state, opt.saved_path + os.sep + "around_1_content_with_title.pkl")
            else:
