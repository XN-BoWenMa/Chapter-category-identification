...
class HierAttNet(nn.Module):
    ...
    def forward(self, title, before1, before2, before3, after1, after2, after3):
        final_list = []

        before_feature3 = self.cnn_net(before3)
        final_list.append(before_feature3.unsqueeze(0))
        
        before_feature2 = self.cnn_net(before2)
        final_list.append(before_feature2.unsqueeze(0))

        before_feature1 = self.cnn_net(before1)
        final_list.append(before_feature1.unsqueeze(0))
        
        title_feature = self.cnn_net(title)
        final_list.append(title_feature.unsqueeze(0))
        
        after_feature1 = self.cnn_net(after1)
        final_list.append(after_feature1.unsqueeze(0))

        after_feature2 = self.cnn_net(after2)
        final_list.append(after_feature2.unsqueeze(0))

        after_feature3 = self.cnn_net(after3)
        final_list.append(after_feature3.unsqueeze(0))
