...
class WordAttNet(nn.Module):
    def __init__(self, word2vec_path, hidden_size=50, dropout=0.5, n_layers=1):
        ...
        dict = dict.astype(np.float16)
        dict_len, embed_size = dict.shape
        dict_len += 1
        #将<unk>、<pad>标签默认为0向量
        pad_word = np.zeros((1, embed_size))
        unknown_word = np.zeros((1, embed_size))
        dict = torch.from_numpy(np.concatenate([unknown_word, dict], axis=0).astype(np.float16))#将unknown_word拼接到dict前面，索引为1
        dict = torch.from_numpy(np.concatenate([pad_word, dict], axis=0).astype(np.float16))#将pad_word拼接到dict前面，索引为0
