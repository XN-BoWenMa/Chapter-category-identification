...
class MyDataset(Dataset):
    ...
    def __init__(self, data_path, dict_path, max_length_sentences=30, max_length_word=35):
        ...
        texts, labels, around_before, around_after = [], [], [], []
        ...
        for row in rows:
            ...
            if len(row[5])>0:
                around_after.append(row[5].split("\t"))
            else:
                around_after.append("")
            texts.append(text)
            labels.append(label)
        ...
        self.around_after = around_after
    ...
    def __getitem__(self, index): #获取每一条数据 #并进行编码
        ...
        if len(after)>0:
            after_encode = [ 
                [self.dict.index(word) if word in self.dict else -2 for word in sentences.split(" ")] for sentences
                in after] #按照dict索引将文档进行编码
            for sentences in after_encode:
                if len(sentences) < self.max_length_word:
                    extended_words = [-1 for _ in range(self.max_length_word - len(sentences))]
                    sentences.extend(extended_words)
            if len(after_encode) < self.max_length_sentences:
                extended_sentences = [[-1 for _ in range(self.max_length_word)] for _ in
                                    range(self.max_length_sentences - len(after_encode))]
                after_encode.extend(extended_sentences)
            after_encode = [sentences[:self.max_length_word] for sentences in after_encode][
                            :self.max_length_sentences]
        else:
            for i in range(self.max_length_sentences):
                vector = [-1 for j in range(self.max_length_word)]
                after_encode.append(vector)
        after_encode = np.stack(arrays=after_encode, axis=0) 
        after_encode += 2
        #---------------------------------------------------------
        document_encode = [ 
            [self.dict.index(word) if word in self.dict else -2 for word in sentences.split(" ")] for sentences
        ...
        return document_encode.astype(np.int64), label, before_encode.astype(np.int64), after_encode.astype(np.int64)
