...
filename = '..\..\data\output\citation_feature.csv'
                ...
                # if sections_list[i].tag=="sectionHeader" and (attri_dic["genericHeader"] != "references"):
                    ...
                    while sections_list[q].tag!="sectionHeader":
                        if sections_list[q].tag in list_sign:
                            cite_num = 0#统计该章节的引用数
                            text.append(sections_list[q].text.strip())#将章节文本按每个标签取下来
                            each_text = sections_list[q].text.strip().lower()
                            #基于规则，统计引用数
                            pattern = re.compile(r'[\(\[][^\(\)\[\]]*?[\)\]]')
                            pattern2_1 = re.compile(r'^[\d,]+$')
                            pattern2_2 = re.compile(r'^[\d]{1,3}$')
                            pattern3_1 = re.compile(r'[1-2][0-9]{3}[a-z]?')
                            pattern3_2 = re.compile(r'[A-Z][^\d]+?[0-9]{2}$')
                            kuohao = pattern.findall(each_text)
                            for each_kuohao in kuohao:
                                pre_divide = each_kuohao[1:-1]
                                kuohao_sign = 0
                                if each_kuohao[0]=="[" and each_kuohao[-1]=="]":
                                    kuohao_sign = 1
                                    #若分号在字符串中，则按分号切分计数
                                if ";" in pre_divide:
                                    for each_fenhao in pre_divide.split(";"):
                                        num_each = pattern3_1.findall(each_fenhao)
                                        cite_num = cite_num+len(num_each)
                                    # print(pre_divide)
                                else:
                                    match2_1 = pattern2_1.search(pre_divide)
                                    match2_2 = pattern2_2.search(pre_divide)
                                    if match2_1!=None:
                                        if not (match2_2!=None and kuohao_sign==0):
                                            cite_num = cite_num+len(pre_divide.split(","))
                                            # print(pre_divide)
                                    else:
                                        divided = pre_divide.split(",")
                                        for each_divided in divided:
                                            num_3_1 = pattern3_1.findall(each_divided)
                                            num_3_2 = pattern3_2.findall(each_divided)
                                            if len(num_3_1)!=0:
                                                cite_num = cite_num+len(num_3_1)
                                                # print(pre_divide)
                                            elif len(num_3_2)!=0:
                                                cite_num = cite_num+len(num_3_2)
                                                # print(pre_divide)
                        if (q+1)<=(len(sections_list)-1):
                            ...
                            break
                    #保存一个章节的内容
                    sec_extract.append([attri_dic["genericHeader"],text,cite_num])   
        except OSError:
            pass
            print("跳过"+flie_ID)
