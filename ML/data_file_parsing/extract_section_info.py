...
filename = '..\..\data\output\ACL_articles_data.csv'
                ...
                # if sections_list[i].tag=="sectionHeader" and (attri_dic["genericHeader"] != "references"):
                    ...
                    while sections_list[q].tag!="sectionHeader":
                        ...
                        if (q+1)<=(len(sections_list)-1):
                            ...
                            break
                    #保存一个章节的内容
                    sec_extract.append([attri_dic["genericHeader"],text,section_title])   
        except OSError:
            pass
            print("跳过"+flie_ID)
        ...
        for i in range(0,length):
            if len(sec_extract[i][1])>0:
                if sec_extract[i][0] in four_section:
                    info = [flie_ID,sec_extract[i][0],sec_extract[i][2],sec_extract[i][1]]
                else:
                    info = [flie_ID,"evaluation and result",sec_extract[i][2],sec_extract[i][1]]
